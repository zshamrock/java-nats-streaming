...
public interface Connection extends AutoCloseable {
  ...
  /**
   * Publishes the payload specified by {@code data} to the subject specified by {@code subject} and
   * asynchronously processes the ACK or error state via the supplied {@code AckHandler}
   * 
   * @param subject the subject to which the message is to be published
   * @param data the message payload
  ...
  /**
   * Publishes the payload specified by {@code data} to the subject specified by {@code subject} and
   * asynchronously processes the ACK or error state.
   * 
   * @param subject the subject to publish the message to
   * @param reply the subject to which subscribers should send responses
   * @param data the message payload
  ...
  /**
   * Creates a {@code Subscription} in the queue group specified by {@code queue} with interest in a
   * given subject, assigns the message callback, and immediately starts receiving messages.
   * 
   * @param subject the subject of interest
   ...
   * @throws TimeoutException if the subscription request times out
   */
  Subscription subscribe(String subject, String queue, MessageHandler cb, SubscriptionOptions opts)
      throws IOException, TimeoutException;
